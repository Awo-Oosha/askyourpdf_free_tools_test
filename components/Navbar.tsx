...
const Content = styled.div<{ $whiteBg: boolean }>`
  position: relative;
  display: flex;
  ...
  gap: 24px;
  background: ${(props) =>
    props.$whiteBg ? "rgba(255, 255, 255, 0.80)" : "rgba(13, 13, 13, 0.8)"};
  box-shadow: ${(props) =>
    props.$whiteBg ? "none" : "0px 4px 4px rgba(0, 0, 0, 0.25)"};
  ...
  ul {
    ...
    li {
      font-weight: 500;
      color: ${(props) =>
        props.$whiteBg ? "#1E1E1E" : "rgba(255, 255, 255, 0.6)"};
...
function Navbar({
  ...
  useEffect(() => {
      ...
      ([entry]) => {
        if (headerRef.current) {
          if (entry.isIntersecting) {
            headerRef.current.classList.add("isIntersecting");
          } else {
            headerRef.current.classList.remove("isIntersecting");
          }
        }
      },
      {
    ...
    return () => {
      if (announcementElement) {
        observer.unobserve(announcementElement);
      }
    };
  }, [announcementRef]);
          ...
          <Content $whiteBg={whiteBg ? true : false}>
            ...
            <NavMenu>
              <ul>
                ...
                {router.pathname === "/pdf-to-chat" ? (
                <li onClick={() => handleScrollToSection("Tools")}>
                  <p>
                    <Trans>Tools</Trans>
                  </p>
                </li> 
...
// // million-ignore
// function Navbar({ announcementRef }: { announcementRef?: RefObject<any> }) {
//   const [menuOpen, setMenuOpen] = useState(false);
