...
export default function ToolsExport({ open, setOpen, exportData }: any) {
  ...
  const exportDocument = async (extension: string) => {
    const currentTimestamp = getCurrentTimestamp();
    let filename = `literature_review_${currentTimestamp}`;
    const extensionType =
      extension === "docx"
        ? "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
        : "application/msword";
    const sanitizedData = exportData.replace(/{|}/g, " ");

    const titleParagraph = new Paragraph({
      text: "Literature Review Generated From AskYourPDF",
    ...
    const paragraphs = [
      new Paragraph({
        ...
        children: [new TextRun(sanitizedData), new TextRun("\n\n")],
      }),
    ];
