...
export default function DocumentUpload({
  ...
  const documentsList = useMemo(() => {
    const flattenedDocs = documents?.pages?.flatMap(
      (document: any) => document?.documents
    );
    if (flattenedDocs) {
      return flattenedDocs.filter((document: any) =>
              ...
              <ul>
                ...
                {documentsList.map((document: any) => (
                  <li
                    key={document.doc_id}
                    onClick={() => {
                      if (document.pages > SUMMARY_MAX_PAGES) {
                        alerts.error(
                          t`Document Length Exceeds Limit`,
                          t`Sorry, but this document has over ${SUMMARY_MAX_PAGES} pages. Please select a document with fewer pages to continue.`,
                          7000
                        );
                        return;
                      }
